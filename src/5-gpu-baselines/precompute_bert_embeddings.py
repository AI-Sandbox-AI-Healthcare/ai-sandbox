...
# ---------------- Tokenized file check ----------------
def safe_load_tokenized(base_prefix, fallback="iter1"):
    """Try to load tokenized files, fallback if not available."""
    files = [
        f"{BASE}/tokenized_input_ids_{base_prefix}.npy",
        f"{BASE}/tokenized_attention_masks_{base_prefix}.npy",
        f"{BASE}/tokenized_subject_ids_{base_prefix}.npy",
    ]
    if all(os.path.exists(f) for f in files):
        return base_prefix
    elif fallback and base_prefix != fallback:
        print(f"⚠️ Tokenized files for {base_prefix} not found. Falling back to {fallback}.")
        return fallback
    else:
        raise FileNotFoundError(f"❌ Tokenized files for {base_prefix} not found and no fallback available.")

METRIC_PREFIX = safe_load_tokenized(METRIC_PREFIX)
...
print(f"Notes: {X_notes.shape}, Masks: {M_notes.shape}, Subjects: {len(sid_notes)}")
...
with torch.no_grad():
    for start in tqdm(range(start_idx, N, BATCH_SIZE), 
                      desc=f"Precomputing CLS embeddings ({METRIC_PREFIX})", 
                      ncols=100):
