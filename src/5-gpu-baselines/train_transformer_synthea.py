"""train_transformer_synthea.py
Transformer baseline on structured sequences with shared validation IDs.
Saves metrics, ROC, confusion matrix, and stacking-ready probabilities.
"""
...
class TransformerClassifier(nn.Module):
    def __init__(self, input_dim, model_dim=64, heads=4, layers=2, dropout=0.3):
        ...
        self.fc = nn.Linear(model_dim, 1)  # single logit for binary
...
with ResourceLogger(tag=f"transformer_{METRIC_PREFIX}"):
    ...
    for epoch in range(30):
        ...
        for xb, yb, mb, _ in train_loader:
            xb, yb, mb = xb.to(device), yb.to(device).unsqueeze(1), mb.to(device)
            optimizer.zero_grad()
