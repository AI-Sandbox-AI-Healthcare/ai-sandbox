...
required_cols = {
    "subject_id", "admittime", "binary_label",
...
feature_cols = list(required_cols - {"subject_id", "admittime", "binary_label"}) \
               + [f"tfidf_{t}" for t in tfidf_terms] + topic_cols

missing = ({"subject_id", "admittime", "binary_label"} | set(feature_cols)) - set(df.columns)
if missing:
