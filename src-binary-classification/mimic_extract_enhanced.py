...
USER = os.getenv("MIMIC_USER", "hmfattah")
PASSWORD = os.getenv("MIMIC_PASSWORD", "hmfattah")  # <-- set in .env in practice
...
diag_df = load_csv("../src/DIAGNOSES_ICD.csv", ["subject_id", "hadm_id", "icd9_code"]).dropna(subset=["icd9_code"])
adm_df = load_csv("../src/ADMISSIONS.csv", ["subject_id", "hadm_id", "admittime", "dischtime", "insurance", "admission_type"])
pat_df = load_csv("../src/PATIENTS.csv", ["subject_id", "gender", "dob", "dod", "expire_flag"])
transfers_df = load_csv("../src/TRANSFERS.csv", ["subject_id", "hadm_id", "icustay_id", "intime", "outtime", "curr_careunit"])
services_df = load_csv("../src/SERVICES.csv", ["subject_id", "hadm_id", "curr_service"])
prescriptions_df = load_csv("../src/PRESCRIPTIONS.csv", ["subject_id", "hadm_id", "drug"])
...
def assign_binary_label(row):
    if row['has_chronic_pain'] and not row['has_mental_health']:
        return 1 
    elif row['has_mental_health'] and not row['has_chronic_pain']:
        return 0  
    else:
        return -1  # Explicit class for neither condition

diag_agg['binary_label'] = diag_agg.apply(assign_binary_label, axis=1)
...
final_cols = [
    ...
    'has_chronic_pain', 'co_occurrence', 'binary_label', 'insurance', 'admission_type', 'gender',
...
merged_df = merged_df[merged_df['binary_label'] != -1].copy()
n_after = len(merged_df)
print(f"\n🧹 Dropped {n_before - n_after} rows with binary_label = -1.")
